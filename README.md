...
- **audio alarm** (optional): a [Modulino Buzzer](https://docs.arduino.cc/hardware/modulino-buzzer/)
  connected via Qwiic. A low 3-second tone plays immediately when the alarm
  starts, and again at each `SUMMARY_HOURS` slot (same schedule as the
  summary email) as a daily reminder, until the alarm clears; on recovery, a
  cheerful 2-second tone plays once. The buzzer is entirely optional — if
  it's not connected, the sketch detects that at boot and simply skips the
  sound; email, matrix and status LEDs work either way.
